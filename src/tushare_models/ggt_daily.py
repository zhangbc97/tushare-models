...
class GgtDaily(Base):
    ...
    buy_amount = Column("buy_amount", Float, nullable=True, comment="买入成交金额(亿元)")
    buy_volume = Column("buy_volume", Float, nullable=True, comment="买入成交笔数(万笔)")
    sell_amount = Column("sell_amount", Float, nullable=True, comment="卖出成交金额(亿元)")
    sell_volume = Column("sell_volume", Float, nullable=True, comment="卖出成交笔数(万笔)")
...
try:
    ...
    setattr(GgtDaily.__table__, "engine", engines.ReplacingMergeTree(order_by=GgtDaily.__primary_key__))
except Exception:
    pass
...
try:
    ...
    GgtDaily.__table__.dialect_options["starrocks"].update(  # type: ignore
        {
            "primary_key": ",".join(GgtDaily.__primary_key__),
            "order_by": ",".join(GgtDaily.__primary_key__),
        }
    )
except Exception:
    pass
...
try:
    ...
    GgtDaily.__table__.dialect_options["databend"].update(  # type: ignore
        {
            "cluster_by": GgtDaily.__primary_key__,
        }
    )
except Exception:
    pass
...
# Doris引擎配置
try:
    GgtDaily.__table__.dialect_options["doris"].update(  # type: ignore
        {
            "unique_key": GgtDaily.__primary_key__,
        }
    )
except Exception:
    pass
