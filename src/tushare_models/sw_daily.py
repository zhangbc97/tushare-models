...
class SwDaily(Base):
    ...
    __api_info_title__: ClassVar[str] = "申万行业指数日行情"
    ...
    __api_params__: ClassVar[Dict[str, Any]] = {
        "ts_code": {"type": "String", "required": False, "description": "行业代码"},
    ...
    name = Column("name", String(), nullable=True, comment="指数名称")
    ...
    amount = Column("amount", Float, nullable=True, comment="成交额(万元)")
    pe = Column("pe", Float, nullable=True, comment="市盈率")
    pb = Column("pb", Float, nullable=True, comment="市净率")
    float_mv = Column("float_mv", Float, nullable=True, comment="流通市值(万元)")
...
try:
    ...
    setattr(SwDaily.__table__, "engine", engines.ReplacingMergeTree(order_by=SwDaily.__primary_key__))
except Exception:
    pass
...
try:
    ...
    SwDaily.__table__.dialect_options["starrocks"].update(  # type: ignore
        {
            "primary_key": ",".join(SwDaily.__primary_key__),
            "order_by": ",".join(SwDaily.__primary_key__),
        }
    )
except Exception:
    pass
...
try:
    ...
    SwDaily.__table__.dialect_options["databend"].update(  # type: ignore
        {
            "cluster_by": SwDaily.__primary_key__,
        }
    )
except Exception:
    pass
...
# Doris引擎配置
try:
    SwDaily.__table__.dialect_options["doris"].update(  # type: ignore
        {
            "unique_key": SwDaily.__primary_key__,
        }
    )
except Exception:
    pass
